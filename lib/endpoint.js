...
function parseSockaddr(addr, defaultPort = 0) {
  ...
  if (net.isIP(addr) === 6)
    return { host: addr.toLowerCase(), port: defaultPort }

  if ((match = /^(\d{1,3}(?:\.\d{1,3}){3})(?::(\d+))?$/.exec(addr)))
    return {
      host: match[1],
      port: match[2] !== undefined ? parseInt(match[2], 10) : defaultPort,
    }
...
class Endpoint {
  ...
  async bind(server, opts, fsImpl = fs) {
    ...
    if (this.path) {
      opts.path = this.path
      await fsImpl.rm(this.path, { force: true })
    } else {
      opts.host = this.host
      opts.port = this.port
    }
    ...
    return new Promise((resolve, reject) => {
      const onListening = async () => {
        if (server.off) server.off('error', onError)
        try {
          if (mode) await fsImpl.chmod(opts.path, mode)
          resolve()
        } catch (chmodErr) {
          reject(chmodErr)
        }
      ...
      const onError = (err) => {
        if (server.off) server.off('listening', onListening)
        reject(err)
      }
...
// Factory: accepts a string addr, a number port, or an already-parsed object.
// Returns a populated Endpoint, or an Error (NOT thrown) on parse failure —
// matches the historical Haraka contract.
