...
exports.get_implicit_mx = async (domain) => {
  ...
  const r = await Promise.allSettled([dns.resolve6(domain), dns.resolve4(domain)])
...
// duplicated from utils, to avoid circular dependency
function sanitize(str, { replacement = '' } = {}) {
...
function normalizeDomain(raw_domain) {
  ...
  } else if (ascii && ascii !== domain) {
    domain = ascii
    console.log(`\tACE encoded '${sanitize(raw_domain)}' to '${domain}'`)
  }
