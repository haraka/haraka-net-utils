...
async function get_public_ip_impl() {
  ...
  try {
    ...
    // Optional dependency. Without it, skip public IP detection — same
    // contract as an unreachable STUN server below: cache null, never throw.
    console.info(
      'get_public_ip: optional dependency @msimerson/stun is not installed, skipping STUN detection. Install it in your app directory ("npm install @msimerson/stun") or declare public_ip in smtp.ini.',
    )
    return this.public_ip
  }
