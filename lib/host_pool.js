  ...
  constructor(hostports_str, retry_secs, opts = {}) {
    const hosts = (hostports_str || '')
      ...
      .map((hostport) => {
        ...
        // Always numeric — keeps host.port consistent across entries and
        // avoids string/number mismatches in `${host}:${port}` dead-host keys.
        return { host: split[0], port: parseInt(split[1], 10) || 25 }
      })
